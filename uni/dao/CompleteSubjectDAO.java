...
public class CompleteSubjectDAO {
	/**
	 * 과목 코드를 이용하여 신청된 과목정보 반환 (이미 성적이 부여된 과목만을 출력한다.)
	 * @param subject_code
	 * @param term
	 * @return
	...
	public List<CompleteSubject> selectByCodeTerm(String subject_code, String term) throws NotFoundException {
		...
		try {
			...
			String selectByCodeSQL = "SELECT student_id, grade_point, subject_date" + 
					" , score" + 
					" , name, pwd, major_number, accumulated_grade, class_level" + 
					" , major_name, necessary_grade" + 
			...
			while(rs.next()) {
				Major major = new Major(rs.getInt("major_number"), rs.getString("major_name"), rs.getInt("necessary_grade"));
				Student student = new Student(rs.getInt("student_id"), rs.getString("name"), rs.getString("pwd"), major, rs.getInt("accumulated_grade"), rs.getInt("class_level"));
				Grade grade = new Grade(rs.getString("grade_point"), rs.getInt("score"));
				Subject subject = new Subject(rs.getString("subject_code"), rs.getString("subject_name"), rs.getInt("credit"), rs.getInt("limit"), rs.getString("start_time"), rs.getInt("major_number"),rs.getInt("run_time"));
				CompleteSubject cs = new CompleteSubject(student, subject, grade, rs.getString("subject_date"));
				subjectList.add(cs);
			}
			if(subjectList.size() == 0) {
				throw new NotFoundException("해당 과목의 " + term + "학기의 수강생이 존재하지 않습니다.");
			}
			...
			e.printStackTrace();
			throw new NotFoundException("과목코드 찾기 오류");
		} finally {
	...
	public void update(CompleteSubject completeSubject) throws ModifyException {
		...
		try {
			con = MyConnection.getConnection();
			String updateSQL = "UPDATE complete_subject SET grade_point = ? WHERE student_id = ? AND com_subject_code = ?";
	...
	public List<CompleteSubject> selectById(int id) throws NotFoundException{
		...
		try{
			con = MyConnection.getConnection();
			String selectByIdSQL = "SELECT student_id, grade_point, subject_date" + 
					" , score" + 
					" , name, pwd, major_number, accumulated_grade, class_level" + 
					" , major_name, necessary_grade" + 
					" , subject_code, subject_name, credit, limit, start_time, run_time" + 
	...
	/**
	 * 수강신청용 메소드
	 * complete_subject에 수강신청한 학생의 내역을 추가시켜준다.
	 * @param cs
	 * @throws AddException
	 */
	...
	public CompleteSubject selectByIdCode(int id, String code) throws NotFoundException{
		...
		try {
			con = MyConnection.getConnection();
			String selectByIdCodeSQL = "SELECT student_id, grade_point, subject_date" + 
					" , score" + 
					" , name, pwd, major_number, accumulated_grade, class_level" + 
					" , major_name, necessary_grade" + 
					" , subject_code, subject_name, credit, limit, start_time, run_time" + 
	...
	/**
	 * 수강신청내역 삭제 메소드
	 * id,code에 해당하는 수강내역을 삭제한다.
	 * 만약 이미 성적이 있는경우 삭제 불가
	 * @param id
	 * @param code
	...
	public void delete(int id, String code) throws DeleteException{
		...
		PreparedStatement pstmt = null;
		try {
			CompleteSubject cs = selectByIdCode(id, code);
			con = MyConnection.getConnection();
			...
			throw new DeleteException("삭제오류 : ");
		}finally {
			MyConnection.close(con, pstmt, null);
		}
	}
